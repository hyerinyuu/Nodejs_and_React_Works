...
// 서버와 통신을 할 때 사용할 여러 설정값을 지정할 변수
...
class BBsMain extends Component {
  ...
  */
  ...
  render() {
    /*
    자식 component에게 state변수를 전달하려면
    변수명 = {this.state} 형식으로 보내고
    자식 component에서는 this.props.변수명 형식으로 
    변수를 사용해아한다
...
/*
    부모 container와 자식 container가 데이터를 주고받는 형식

    부모 >> 자식 식의 일방통행 방식으로 데이터를 전달할 수 있다.
    부모 >> 자식으로 전달된 변수는
    자식 입장에서는 모두 props가 된다.

    ※ props 형태의 변수는 절대 변경할 수 없는 형태가 되고
      단순히 rendering하는 용도로만 사용해야 한다.

    변화가 있는 데이터를 자식이 rendering 하도록 하기 위해서는
    부모 container에서 state 형식의 변수로 만들고
    어떤 형태를 통하여 데이터를 변경하면 
    그 데이터가 자식에게 전파되어 화면에 변화된 모습을 rendering한다.
