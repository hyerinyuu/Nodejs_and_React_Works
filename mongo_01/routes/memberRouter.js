// web에서 request한 path 관련 정보를 추출하기 위한 도구(middle ware)를 선언
var express = require("express")
var router = express.Router()

// mongoDB에 CRUD를 구현하기 위한 VO객체 선언
...
router.post("/insert", function(req, res){
    /*
    WEB으로부터 데이터 전달받기
     - QUERY문자열(?변수=값&변수1=값)로 보낸 데이터
        req.query.변수 형식으로 받는다.

     - path valiable(/path/:변수1/:변수2) 형식으로 받는 데이터
        req.params.변수1 형식으로 받는다.

    form에 담겨서 전달되는 데이터는 req.body에 담겨서 vo 형식으로 받는다.

    req.body에 담겨져 온 데이터를 
    newVO 변수에 복사하고
    save() method를 실행한 후 정상적으로 save()가 되면 
    callback 함수를 실행하여 
